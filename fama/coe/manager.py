...
class CoEManager:
    ...
    def format_top_chains(self, max_expr_chars: Optional[int] = None) -> list[str]:
        ...
        selected = ranked[: self.prompt_chains] if self.prompt_chains else ranked
        formatted: list[str] = []
        for chain in selected:
            ...
            for expr in chain.expressions:
                ...
                expressions.append(f"{label} (ric={score:.3f}, benchmark_assets={bench_label})")
            if expressions:
    ...
    def integrate_factor(
        ...
        if chain is None or factor_series.empty:
            return None
        result = self._compute_single_rankic(factor_series)
        if result is None:
            return None
    ...
    def _compute_rankic_scores(self, factor_frame: pd.DataFrame, expr_to_name: dict[str, str]) -> dict[str, float]:
        ...
        for expr in factor_frame.columns:
            ...
            per_asset = self._compute_asset_scores(series)
    ...
    def _compute_asset_scores(self, series: pd.Series) -> dict[str, float]:
        scores: dict[str, float] = {}
        ...
        for asset in candidates:
            ...
            if series_clean.nunique() <= 1 or returns_clean.nunique() <= 1:
                continue
            try:
                ric = self._ric_calc.efficent_cal_ric(
                    joined.iloc[:, 0].to_numpy(),
                    joined.iloc[:, 1].to_numpy(),
                )
            except Exception:
                continue
            if pd.isna(ric):
    ...
    def _compute_single_rankic(self, series: pd.Series) -> Optional[tuple[float, Optional[str], dict[str, float]]]:
        per_asset = self._compute_asset_scores(series)
        if not per_asset:
            return None
        best_asset, best_score = max(per_asset.items(), key=lambda item: abs(item[1]))
