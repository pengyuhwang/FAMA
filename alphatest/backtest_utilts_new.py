...
# 起始日期默认与牛魔王保持一致
def backtest_nmw(
    target, price_df, start_date=None, end_date=None, D=10, fillna_asset="empty", skip_empty_warehouse=False, fee=0.0
):
    # 确保索引类型为 datetime
    target.index = pd.to_datetime(target.index)
    price_df.index = pd.to_datetime(price_df.index)
...
def prepare_price_data(
    common_price_path=None, au9999_price_path=None, data_path="nmw/data/ifind_updating/fof_price_updating.parquet"
):
    ...
    # 准备当日收盘价数据
    working_days = native_price[native_price.unique_id == "000300.SH"].set_index("time").sort_index().index
    price_df = (
...
if __name__ == "__main__":
    ...
    native_price, price_df, open_price_df, working_days = prepare_price_data(
        data_path="nmw/data/ifind_updating/fof_price_updating.parquet"
    )
